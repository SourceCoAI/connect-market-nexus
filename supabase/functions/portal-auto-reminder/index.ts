 ...
 *
 * Cron-triggered (daily). Finds stale deal pushes across portal orgs that
 * have auto-reminders enabled and queues reminder notifications. The
 * portal-notification-processor cron job picks them up and sends emails.
 *
 * Logic per org with auto_reminder_enabled = true:
 *   - Find pushes in pending_review/viewed status
 *   - Where created_at < (now - auto_reminder_days)
 *   - Where reminder_count < auto_reminder_max
 *   - Queue a reminder notification + bump reminder_count
 *
 * No auth required (cron-triggered, uses service role).
 */
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (orgsError) {
      ...
      return new Response(JSON.stringify({ error: 'Failed to fetch portal organizations' }), {
        status: 500,
    ...
    if (!orgs || orgs.length === 0) {
      return new Response(JSON.stringify({ orgs_checked: 0, reminders_queued: 0 }), {
        status: 200,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    for (const org of orgs) {
      ...
      if (pushesError) {
        console.error(
          `[portal-auto-reminder] Failed to fetch pushes for org ${org.id}:`,
          pushesError,
        );
        continue;
      }
      ...
      for (const push of stalePushes) {
        const dealHeadline =
          ((push.deal_snapshot as Record<string, unknown>)?.headline as string) || 'a deal';
        ...
        // Queue reminder notifications (sent_at = null, processor handles delivery)
        ...
        const body = [
          ...
          ``,
          `Please log in to your portal to take a look.`,
        ].join('\n');
        ...
        if (notifError) {
          console.error(
            `[portal-auto-reminder] Failed to insert notifications for push ${push.id}:`,
            notifError,
          );
          continue;
        }
        ...
        totalRemindersQueued += activeUsers.length;
      }

      console.log(
        `[portal-auto-reminder] Org ${org.id} (${org.name}): processed ${stalePushes.length} stale push(es)`,
      );
    }

    console.log(
      `[portal-auto-reminder] Complete: ${orgs.length} org(s) checked, ${totalRemindersQueued} reminder(s) queued`,
    );

    return new Response(
      JSON.stringify({ orgs_checked: orgs.length, reminders_queued: totalRemindersQueued }),
