...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (resultEmail) {
      ...
      await supabase.from('enriched_contacts').upsert(
        {
          ...
          company_name: request.company_name || request.domain || 'Unknown',
          ...
          confidence: 'medium',
          source: 'clay_name_domain',
          enriched_at: new Date().toISOString(),
          search_query: `clay:${request.first_name} ${request.last_name}@${request.domain}`,
        },
      ...
      console.log(
        `[clay-webhook-name-domain] Enrichment saved — email: ${resultEmail} for ${fullName}`,
      );
    } else {
      console.log(`[clay-webhook-name-domain] No email found by Clay for request: ${requestId}`);
    }
