...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!['anonymous_teaser', 'full_memo', 'data_room'].includes(documentCategory)) {
      return new Response(
        JSON.stringify({
          error: 'document_category must be anonymous_teaser, full_memo, or data_room',
        }),
    ...
    if (!ALLOWED_TYPES.has(file.type)) {
      return new Response(
        JSON.stringify({
          error: `File type ${file.type} not allowed. Allowed: PDF, DOCX, XLSX, PPTX, JPG, PNG, CSV`,
        }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
      );
    }
    ...
    const categoryPath =
      documentCategory === 'anonymous_teaser'
        ? 'teasers'
        : documentCategory === 'full_memo'
          ? 'memos'
          : 'data-room';
