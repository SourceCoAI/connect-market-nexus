...
// Valid columns in buyers table (verified against actual schema)
  ...
  input_schema: {
    ...
    properties: {
      ...
      services_offered: {
        type: 'string',
        description:
          'Detailed description of the services offered. Write as a natural prose paragraph covering: what specific services they provide, revenue mix between service lines if mentioned, whether residential vs commercial, and any specializations. NOT a list — write 2-4 sentences.',
      },
      business_summary: {
        type: 'string',
        description: '2-3 sentence overview of what the company does operationally',
      },
      business_type: {
        type: 'string',
        enum: ['Service Provider', 'Distributor', 'Manufacturer', 'Retailer', 'Software', 'Other'],
      },
      industry_vertical: {
        type: 'string',
        description: "Industry category (e.g., 'Home Services - HVAC')",
      },
      revenue_model: {
        type: 'string',
        description:
          'Detailed description of how the company generates revenue — include contract types, customer payment models, recurring vs project-based work, franchise fees if applicable. Write 1-3 sentences.',
      },
    },
  },
};
  ...
  input_schema: {
    ...
    properties: {
      primary_customer_size: {
        type: 'string',
        description:
          "Target customer size (e.g., 'Residential homeowners', 'Mid-market businesses')",
      },
      customer_industries: {
        type: 'array',
        items: { type: 'string' },
        description: "Industries served (e.g., ['Residential', 'Commercial', 'Industrial'])",
      },
      customer_geographic_reach: {
        type: 'string',
        description: "Geographic scope of customers (e.g., 'Southeast US', 'National')",
      },
  ...
  input_schema: {
    ...
    properties: {
      ...
      geographic_footprint: {
        ...
        items: { type: 'string' },
        description:
          'Array of 2-letter state codes where company operates, has offices, or provides services. Include states from office addresses, service area lists, explicitly named states, and cities with known states.',
      },
      service_regions: {
        type: 'array',
        items: { type: 'string' },
        description:
          'All states where company provides services or has customers, as 2-letter codes. Superset of geographic_footprint.',
      },
      operating_locations: {
        type: 'array',
        items: { type: 'string' },
        description:
          'REQUIRED: Every physical office, branch, store, or location mentioned on the website as "City, ST" (e.g., "Dallas, TX", "Atlanta, GA"). Extract from addresses, location lists, footer, contact pages, and branch directories. This is a critical field — extract aggressively.',
      },
      number_of_locations: {
        type: 'integer',
        description:
          'Total number of physical locations/stores/branches/offices. Extract from explicit mentions like "100+ locations", "serving 50 markets", or count from location directory pages. If not explicitly stated, count the operating_locations extracted.',
      },
    },
  },
};
...
export const PROMPT_3B_PE_INTELLIGENCE = {
  ...
  description:
    'Extract PE firm identity, acquisition history, activity, and portfolio from website content',
  input_schema: {
    type: 'object',
    properties: {
      // PE Firm Identity
      pe_firm_name: {
        type: 'string',
        description: 'Official name of the PE / investment firm as stated on the website',
      },
      buyer_type: {
        type: 'string',
        enum: [
        ...
        description:
          "Classify this buyer: 'private_equity' (PE fund), 'corporate' (strategic/operating company including PE-backed platforms), 'family_office', 'independent_sponsor', 'search_fund', or 'individual_buyer'",
      },
      // Acquisition History
      recent_acquisitions: {
        type: 'array',
      ...
      total_acquisitions: {
        type: 'integer',
        description: 'Total number of acquisitions mentioned',
      },
      ...
      target_industries: {
        type: 'array',
        items: { type: 'string' },
        description: 'Industries the PE firm focuses on',
      },
      target_services: {
        type: 'array',
        items: { type: 'string' },
        description: 'Service types of interest',
      },
      acquisition_appetite: {
        type: 'string',
        description: "Activity level, e.g., 'Very Active - 15-20 deals annually'",
      },
      // Portfolio
      portfolio_companies: {
        type: 'array',
        items: { type: 'string' },
        description: 'List of portfolio company names',
      },
...
export function validateGeography(extracted: AIExtractionResult): AIExtractionResult {
  ...
  const hasAnyGeoEvidence =
    (data.geographic_footprint?.length || 0) > 0 ||
  ...
  if (data.operating_locations && Array.isArray(data.operating_locations)) {
    ...
    if (locStates.length > 0) {
      ...
      console.log(
        `Added ${locStates.length} states from operating_locations to geographic_footprint`,
      );
    }
  }
  ...
  if (
    (!data.geographic_footprint || data.geographic_footprint.length === 0) &&
  ...
  if (data.hq_state) {
    ...
    if (VALID_STATE_CODES.has(data.hq_state)) {
      ...
      if (data.hq_city) {
        ...
        if (
          !data.operating_locations.some((l: string) => l.toLowerCase() === hqLoc.toLowerCase())
        ) {
  ...
  const extractedCount =
    typeof data.number_of_locations === 'number' ? data.number_of_locations : 0;
  ...
  if (actualCount > 0 || extractedCount > 0) {
    ...
    console.log(
      `Location count: extracted=${extractedCount}, actual=${actualCount}, using=${data.number_of_locations}`,
    );
  }
...
export function shouldOverwrite(
  ...
  if (existing.priority > 0 && existing.priority > newPriority) {
    console.log(
      `🛡️ PRIORITY: Skipping ${fieldName} — existing source "${existing.sourceType}" (priority ${existing.priority}) > new source "${newSourceType}" (priority ${newPriority})`,
    );
    return false;
  }
  ...
  if (TRANSCRIPT_PROTECTED_FIELDS.includes(fieldName) && hasTranscriptSource) {
    if (newSourceType === 'platform_website' || newSourceType === 'pe_firm_website') {
      console.log(
        `🛡️ PROTECTED FIELD: Skipping ${fieldName} — transcript-protected and buyer has transcript data`,
      );
      return false;
    }
  }
...
export function buildBuyerUpdateObject(
  ...
  const existingFieldSources: Record<string, { source: string; priority: number; at: string }> =
    ((
      existingSources.find((s) => (s as Record<string, unknown>).type === 'field_sources') as
        | Record<string, unknown>
        | undefined
    )?.fields as Record<string, { source: string; priority: number; at: string }>) || {};
  ...
  for (const [rawField, value] of Object.entries(extractedData)) {
    ...
    if (
      !shouldOverwrite(field, buyer[field], value, hasTranscriptSource, existingSources, sourceType)
    ) {
      continue;
    }
    ...
    if (Array.isArray(value)) {
      ...
      if (
        field === 'geographic_footprint' ||
        field === 'service_regions' ||
        field === 'target_geographies'
      ) {
      ...
      if (normalized.length > 0) {
        ...
        if (
          MERGE_FIELDS.has(field) &&
          Array.isArray(buyer[field]) &&
          (buyer[field] as string[]).length > 0
        ) {
          const existing = buyer[field] as string[];
          const isStateCodes = field === 'geographic_footprint' || field === 'service_regions';
          const seen = new Set(
            existing.map((v) => (isStateCodes ? v.toUpperCase() : v.toLowerCase())),
          );
          ...
          console.log(
            `Merged ${field}: ${(buyer[field] as string[]).length} existing + ${normalized.length} new → ${unique.length} total`,
          );
        } else {
