...
RETURNS TRIGGER AS $$
BEGIN
  -- If pe_firm_name is set, the buyer is a platform company, not a PE firm
  IF NEW.pe_firm_name IS NOT NULL AND NEW.pe_firm_name != '' AND NEW.buyer_type = 'private_equity' THEN
    NEW.buyer_type := 'corporate';
    NEW.is_pe_backed := true;
  END IF;

  -- Also enforce when parent_pe_firm_id is set
