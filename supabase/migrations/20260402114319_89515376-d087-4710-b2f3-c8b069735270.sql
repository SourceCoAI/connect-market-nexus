...
-- Update enhanced_merge_or_create_connection_request to require at least ONE agreement (NDA or Fee)
-- instead of NDA-only.
...
BEGIN
  ...
  -- SERVER-SIDE GATE: Enforce at least one agreement (NDA OR Fee Agreement)
  ...
  IF v_user_email IS NOT NULL THEN
    ...
    IF NOT COALESCE(v_nda_coverage.is_covered, false) AND NOT COALESCE(v_fee_coverage.is_covered, false) THEN
      RAISE EXCEPTION 'At least one agreement (NDA or Fee Agreement) must be signed before requesting deal access';
    END IF;
  END IF;
  ...
  -- Check for existing lead-based request
