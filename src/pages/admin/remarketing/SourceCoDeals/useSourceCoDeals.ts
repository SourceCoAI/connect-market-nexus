...
import { SOURCECO_FIELDS } from '@/components/filters';
import {
  useGlobalGateCheck,
...
export function useSourceCoDeals() {
  ...
  } = useFilterEngine(deals ?? [], SOURCECO_FIELDS);
    ...
    async (mode: 'unenriched' | 'all') => {
      ...
      try {
        const result = await startOrQueueMajorOp({
          ...
          description: `Enriching ${targets.length} SourceCo deals`,
          userId: user?.id || '',
          contextJson: { source: 'sourceco' },
        });
    ...
    async (mode: 'unscored' | 'all') => {
      ...
      try {
        const result = await startOrQueueMajorOp({
          operationType: 'deal_enrichment',
          totalItems: targets.length,
          description: `Scoring ${targets.length} SourceCo deals`,
          userId: user?.id || '',
          contextJson: { source: 'sourceco_scoring' },
        });
        activityItem = result.item;
      } catch {
        /* Non-blocking */
    ...
    async (dealIds: string[], options?: { force?: boolean }) => {
      ...
      try {
        const result = await startOrQueueMajorOp({
          ...
          description: `Enriching ${dealIds.length} SourceCo deals`,
          userId: user?.id || '',
          contextJson: { source: 'sourceco_selected' },
        });
        activityItem = result.item;
      } catch {
        /* Non-blocking */
  ...
  const handleAssignOwner = useCallback(
    async (dealId: string, ownerId: string | null) => {
      ...
      queryClient.setQueryData(
        ['remarketing', 'sourceco-deals'],
        (old: SourceCoDeal[] | undefined) =>
          old?.map((deal) =>
            deal.id === dealId
              ? {
                  ...deal,
